...
export const openAI = onDocumentCreated(
    ...
    if (status === "failed") {
      ...
      await setDoc(
        quizzesRef,
        {
          success: false,
          loading: false,
          quiz: "There was an error generating your quiz.",
        },
        { merge: true }
      );
      const result = await openai.beta.threads.runs.retrieve(thread.id, run.id);
      ...
      const quiz = (messages.data[0].content[0] as { text: { value: string } })
        .text.value;
      ...
      await setDoc(
        quizzesRef,
        { success: true, loading: false, quiz: quiz },
        { merge: true }
      );
    }
  }
