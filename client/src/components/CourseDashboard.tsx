...
const [quiz, setQuiz] = useState<string>('');
  ...
  useEffect(() => {
    ...
    const requestQuery = query(collection(db, "users", currentUser!.uid, "quizzes"), orderBy('createdAt', 'desc'), limit(1));
    console.log(requestQuery);
   // const quizQuery = currentUser ? query(collection(db, "users", currentUser.uid, "requests"), orderBy('createdAt', 'desc'), limit(1)) : '';

    const unsubscribe = onSnapshot(requestQuery, (snapshot: { docChanges: () => any[]; }) => {
 ...
 // setQuiz(quiz.replace(/^```json\s*|\s*```$/g, ''));
  let extract = quiz.replace(/^```json\s*|\s*```$/g, '');
  ...
  return (
    <>
      ...
      <div>
        ...
        {loading ? <div style={{ textAlign: 'center', fontSize: '20px' }}>Generating quiz...</div> : <div>{quizFormat()}</div>}
      </div>
    </>
  );
};
